 ...
 * UPDATED: Increased to 7 days (604800 seconds) to reduce egress by 96%
 * Was causing 779% egress overage with 1-hour expiry
 */
export async function getSignedUrl(filePath: string, expiresIn: number = 604800) {
  try {
    const { data, error } = await supabase.storage
      .from(BUCKET_NAME)
